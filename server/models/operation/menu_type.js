...
Menu_type.getMenuTypeByMenuId = function getMenuTypeByMenuId(menu_id) {
    var sql = "SELECT d1.name, d1.top_id, d2.name as type, menu_type.dictionary_id FROM menu_type LEFT JOIN dictionary d1 ON menu_type.dictionary_id = d1.dictionary_id LEFT JOIN dictionary d2 ON d1.top_id = d2.dictionary_id WHERE menu_type.menu_id = ? AND menu_type.state = 0";
    let params = [menu_id]
    return mysqlHelper.execute1(sql, params)
}

//删除分类
Menu_type.deleteMenuTypeByMenuId = function deleteMenuTypeByMenuId(menu_id) {
    let sql = "DELETE FROM menu_type WHERE menu_id = ?";
    let params = [menu_id]
    return mysqlHelper.execute1(sql, params)
}
