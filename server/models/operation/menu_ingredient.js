...
Menu_ingredient.getMenuIngredientByMenuId = function getMenuByMenuId(menu_id) {
    let sql = "SELECT groupnum,groupname,ingredientname,amount FROM menu_ingredient INNER JOIN ingredient ON ingredient.ingredient_id = menu_ingredient.ingredient_id WHERE menu_ingredient.menu_id = ?";
    let params = [menu_id]
    return mysqlHelper.execute1(sql, params)
}
